...
- The equipment system must be able to create empty and starter loadouts, filter available gear by player level, generate weighted non-duplicated reward choices, return reward choices with slot replacement, slot display, and effect summary details, equip each item into its declared slot, and mirror applied loadouts onto player state.
