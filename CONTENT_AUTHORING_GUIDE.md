...
- `src/systems/equipment.js`: wearable loadout defaults, player-level availability filtering, reward-choice candidates, reward-choice details, equipped-item lookup, slot replacement helpers, and effect delta summaries for future comparison UI.
...
- The equipment system must be able to create empty and starter loadouts, describe every loadout slot, aggregate equipped effect totals, compare candidate effect deltas, localize slot/item/rarity/effect copy, filter available gear by player level, report whether a non-equipped reward exists, generate weighted non-duplicated reward choices, package localized reward offers with pool counts, render equipment reward cards with readable effect deltas, return reward choices with slot replacement, slot display, rarity, reward weight, effect summary details, and effect delta details, apply a selected reward into the loadout, equip each item into its declared slot, and mirror applied loadouts onto player state.
