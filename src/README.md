...
- `systems/theme-surroundings.js`: deterministic 1-4 hex grouping and metadata for non-playable theme mosaics.
...
- The side stats panel renders a compact gear list with one chip per equipment slot plus aggregate effect chips, so new reward choices are visible after selection even before a full inventory screen exists.
...
- If wearable gear changes, keep slot ids unique, provide exactly one starter item per slot, and keep equipment ids, localized English/Spanish copy, planned asset paths, localized rarity/effect labels, minLevel, rarity, optional rewardWeight, registered effect payloads, player-level availability, reward availability, localized reward cadence plans/offers, weighted reward choices, reward selection, bundled replacement/slot/rarity/effect comparison details, equipment reward-card rendering, post-room reward-flow ordering, empty/starter loadouts, loadout slot details, active bonus summaries, equipped effect totals/deltas, and idempotent loadout behavior valid; `smoke-data` treats this as the gear contract until loot is implemented.
...
- Level 2 currently has one common replacement per equipment slot, and level 4 has one rare replacement per slot, so future room rewards can offer early and mid-run gear choices before larger loot tables exist.
...
- `node tools/smoke-data.js` validates asset maps, object-to-sprite metadata, sprite sheet rows/timing, sprite files, fallback colors, room profiles, room objectives/templates, spawn references, content usage paths, content data, supported item effect handlers and payloads, supported enemy behavior types and payload references, inspect stat chips for key behavior hints, terrain, visibility, and route-risk states, relic localization/metadata/reward coverage, wearable gear metadata, planned gear asset paths and artist request coverage, localized gear/rarity/effect copy, slot uniqueness, starter coverage, localized reward cadence plans/offers, equipment reward-card rendering, post-room reward-flow sequencing, loadout slot details, active bonus summaries, equipped effect totals/deltas, and idempotent loadout behavior, deterministic 1-4 hex surroundings placement, theme eligibility for core supplies plus mid/late enemies, run-summary recommendation branches, and cell-interaction role/action metadata.
- `node tools/smoke-browser.js` launches the browser, verifies the Test page renders and starts every generated object scenario, checks effect inspect stat metadata, starts a run, verifies starter equipment and post-room reward sequencing, verifies auto-walk stops on stacked action objects, checks inspect stat chips, and checks market card structure.
...
- The level-1 authored sequence introduces one decision at a time: collect supplies in room 1, spend pollen on a wax door in room 2, flank an armored guard in room 3, then collect water before crossing a single burning-cell gate in room 4. Room 2 supplies pollen before its door, walls every alternate approach to its visible exit, and uses a bilingual `openWaxDoor` objective that names the exchange; missing the pollen is recoverable because a sting can still open the door. Room 3 uses a bilingual `flankGuard` objective that directs the player to its highlighted safe cells before attacking the Thorn Beetle. Room 4 is fully authored with no unrelated pickups, enemies, or hazards; alternate exit approaches are walls, the exit stays revealed, and its bilingual `crossFire` objective states the intended action directly. Missing the water is costly but not a hard lock because the fire remains traversable for one damage.
...
- Room 3's enemy-gate lesson places its armored Thorn Beetle three to four route steps from the entrance, then uses a gold pulsing `lessonGuard` focus ring and green `lessonSafe` cells to pair the visible teaching target with safe flank positions without using danger-red language.
...
- Crawling Fire has a direct tactical counter: interacting while carrying water spends `1 water` and extinguishes it as the turn action without consuming a sting attack. Its Test scenario starts with water and explains this option.
- Guard Wasp uses the data-driven `pushPlayerOnAttack` behavior: a surviving turn hit can displace the bee one empty hex farther away, making nearby landing space part of route planning. The inspect panel labels this as a forced-movement threat.
