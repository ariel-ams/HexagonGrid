...
- If authored room templates change, update `ROOM_TEMPLATE_DEFS` with the lesson's supported metadata keys and required objects/enemies so data smoke can catch schema typos, broken references, random templates that do not fit any dungeon theme, and themes with no compatible random lesson before browser tests.
