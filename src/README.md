...
- `systems/equipment.js`: wearable slot loadout helpers for empty/starter gear, player-level availability, lookup, effects, and slot replacement.
...
- If wearable gear changes, keep slot ids unique, provide exactly one starter item per slot, and keep equipment ids, minLevel, rarity, registered effect payloads, player-level availability, empty/starter loadouts, and loadout behavior valid; `smoke-data` treats this as the gear contract until loot is implemented.
