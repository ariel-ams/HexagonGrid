...
- If wearable gear changes, keep slot ids unique, provide exactly one starter item per slot, and keep equipment ids, localized English/Spanish copy, localized rarity/effect labels, minLevel, rarity, optional rewardWeight, registered effect payloads, player-level availability, reward availability, localized reward offers, weighted reward choices, reward selection, bundled replacement/slot/rarity/effect details, empty/starter loadouts, loadout slot details, and loadout behavior valid; `smoke-data` treats this as the gear contract until loot is implemented.
